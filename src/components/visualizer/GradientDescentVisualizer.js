...
const GRADIENT_TUTORIAL = [
  {
    ...
    body: '"Simple Bowl" is perfectly convex — gradient descent always finds the global minimum. "Bumpy" is non-convex with local minima. "Saddle / Ravine" has tricky geometry. Each shows different optimisation behaviour.',
    highlight: '"Loss Function" dropdown — top-left of controls',
  },
  {
    title: 'Understand the visual',
    body: 'The blue curve is the loss surface L(w). The orange ball is the current weight position w. The purple dashed line is the gradient (tangent slope) at that point — it shows which direction the optimiser will move.',
    highlight: 'Main chart — blue curve with orange ball and purple tangent',
  },
  {
    title: 'Set the learning rate η',
    body: 'Drag the "Learning Rate" slider (0.01–0.5). Too small = convergence is very slow. Too large = ball overshoots the minimum and oscillates. Try 0.45 on the Bumpy surface to see it bounce.',
...
const mapY = (loss, lo, hi) => PAD.t + ((hi - loss) / (hi - lo)) * PLOT_H;
...
export default function GradientDescentVisualizer() {
  ...
  const reset = useCallback(() => {
    ...
    stateRef.current = {
      w: initW,
      history: [{ w: initW, loss: fn.f(initW) }],
      steps: 0,
    };
    lrRef.current = lr;
  }, [fn, lr]);
  ...
  const handleRunPause = () => {
    if (running) {
      ...
      setRunning(true);
      intervalRef.current = setInterval(doStep, 120);
    }
  };
    ...
    <div className="relative w-full h-full p-4 space-y-4">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        <div>
          ...
          <select
            value={fnKey}
            onChange={e => {
              if (!running) setFnKey(e.target.value);
            }}
        ...
        <button
          onClick={!running ? doStep : undefined}
          disabled={running}
          className="py-1.5 px-4 rounded-lg bg-dark-lightest text-gray-200 text-sm border border-dark-lightest hover:border-primary/40 transition-colors disabled:opacity-40"
        >
          Step →
      ...
      <div className="flex flex-wrap gap-6 text-sm">
        <span className="text-gray-400">
          Step: <span className="text-white font-bold">{steps}</span>
        </span>
        <span className="text-gray-400">
          w = <span className="text-primary font-bold">{w.toFixed(4)}</span>
        </span>
        <span className="text-gray-400">
          Loss = <span className="text-accent font-bold">{loss.toFixed(4)}</span>
        </span>
        <span className="text-gray-400">
          ∇ ={' '}
          <span
            className={`font-bold ${
              Math.abs(grad) < 0.01 ? 'text-green-400' : 'text-secondary'
            }`}
          >
            {grad.toFixed(4)}
          </span>
        ...
        {Math.abs(grad) < 0.001 && (
          <span className="text-green-400 font-bold">✓ Converged!</span>
        )}
      </div>

      {/* main chart */}
      <div
        className="rounded-xl overflow-hidden border border-dark-lightest bg-dark shadow-lg"
        style={{ lineHeight: 0 }}
      >
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: 'block' }}>
          ...
          <path
            d={`${curvePath} L${PAD.l + PLOT_W},${PAD.t + PLOT_H} L${PAD.l},${PAD.t + PLOT_H} Z`}
            fill="url(#lossGrad)"
          />
          ...
          {history.slice(-30).map((h, i, arr) => {
            ...
            return (
              <line
                key={i}
                x1={mapX(prev.w, fn)}
                y1={mapY(prev.loss, lo, hi)}
                x2={mapX(h.w, fn)}
                y2={mapY(h.loss, lo, hi)}
                stroke="#F97316"
                strokeWidth={1.5}
                strokeOpacity={0.4 + 0.6 * (i / arr.length)}
              />
            );
          })}
      ...
      {history.length > 2 && (
        <div>
          ...
          <div
            className="rounded-xl border border-dark-lightest bg-dark shadow-lg overflow-hidden"
            style={{ lineHeight: 0 }}
          >
            <svg width="100%" viewBox={`0 0 ${W} ${LH}`} style={{ display: 'block' }}>
              <path d={histPath} fill="none" stroke="#F97316" strokeWidth={2} />
              <text x={PAD.l} y={LH - 4} fontSize={9} fill="#64748b">
                0
              </text>
              <text
                x={W - PAD.r}
      ...
      <div className="bg-dark shadow-lg rounded-xl p-4 border border-dark-lightest text-sm text-gray-400 leading-relaxed">
        ...
        The <span className="text-orange-400">orange ball</span> is the current weight
        position.
        The <span className="text-purple-400">purple line</span> shows the gradient
        (tangent slope).
        <br />
        <span className="text-white">{fn.desc}</span>
      </div>
